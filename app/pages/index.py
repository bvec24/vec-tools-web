...
def top_bar() -> rx.Component:
    ...
    return rx.el.header(
        rx.el.div(
            ...
            rx.el.h1("VEC Tools", class_name="text-xl font-bold text-gray-800"),
        ...
        rx.el.div(
            rx.el.div(
                ...
                rx.el.button(
                    ...
                    class_name=rx.cond(
                        ...
                        "w-10 h-6 rounded-full p-1 bg-gray-200 transition-all",
                    ),
            ...
            rx.el.button(
                ...
                class_name="bg-purple-50 hover:bg-purple-100 text-purple-700 font-semibold px-4 py-2 rounded-lg text-sm transition-colors",
            ),
            class_name="flex items-center gap-6",
        ),
        class_name="fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-sm border-b border-gray-200 flex items-center justify-between px-6 z-30",
    )
...
def sidebar() -> rx.Component:
    ...
    return rx.el.aside(
            ...
            rx.el.nav(
                rx.foreach(
                    ...
                    lambda group_item: rx.el.a(
                        ...
                        rx.el.span(
                            group_item[0], class_name="font-medium text-gray-700"
                        ),
                        href=f"#{group_item[0]}",
                        class_name="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-purple-50 transition-colors",
                    ),
                ),
        ...
        class_name="fixed top-16 bottom-0 left-0 w-64 bg-white border-r border-gray-200 flex flex-col pt-4 z-20",
    )
...
def tool_card(tool: Tool) -> rx.Component:
    ...
    return rx.el.div(
        rx.el.div(
            ...
            rx.el.span(
                tool_var["group"],
                class_name="text-xs font-semibold text-purple-700 bg-purple-100 px-2.5 py-1 rounded-full w-fit",
            ),
            class_name="flex items-center justify-between mb-3",
        ),
        rx.el.h3(
            rx.cond(tool_var["title"] != "", tool_var["title"], tool_var["name"]),
            class_name="font-bold text-gray-800 text-lg mb-1 truncate",
        ),
        rx.el.p(
            rx.cond(tool_var["desc"] != "", tool_var["desc"], tool_var["relpath"]),
            class_name="text-sm text-gray-500 mb-4 h-10 overflow-hidden",
        ),
        rx.el.button(
            "Ejecutar",
            rx.icon(tag="play", class_name="ml-2 h-4 w-4"),
            on_click=lambda: AppState.run_tool(tool_var["relpath"]),
            class_name="w-full bg-gray-800 text-white font-semibold py-2.5 rounded-lg hover:bg-gray-900 transition-colors flex items-center justify-center",
        ),
        class_name="bg-white p-5 rounded-xl border border-gray-200 shadow-md hover:shadow-lg hover:border-purple-300 transition-all duration-300 transform hover:-translate-y-1",
    )
            ...
            rx.radix.primitives.dialog.content(
                ...
                rx.el.div(
                    rx.el.div(
                        rx.el.div(
                            rx.el.h3(
                                "STDOUT", class_name="font-semibold text-gray-700 mb-2"
                            ),
                            rx.el.button(
                                rx.icon(tag="copy"),
                        ...
                        rx.el.pre(
                            rx.el.code(AppState.stdout, class_name="text-xs font-mono"),
                            class_name="bg-gray-50 p-4 rounded-md border max-h-64 overflow-y-auto",
                        ),
                        class_name="mb-4",
                    ),
                    rx.el.div(
                        rx.el.div(
                            rx.el.h3(
                                "STDERR", class_name="font-semibold text-red-700 mb-2"
                            ),
                            rx.el.button(
                                rx.icon(tag="copy"),
                        ...
                        rx.el.pre(
                            rx.el.code(
                                AppState.stderr,
                                class_name="text-xs font-mono text-red-600",
                            ),
                            class_name="bg-red-50 p-4 rounded-md border border-red-200 max-h-64 overflow-y-auto",
                        ),
                    ...
                    rx.el.div(
                        ...
                        rx.el.p(
                            "Ejecutando...", class_name="text-sm text-gray-500 ml-3"
                        ),
...
@rx.page(on_load=[AppState.hydrate_user, AppState.on_load])
            ...
            rx.cond(
                ...
                rx.foreach(
                    ...
                    lambda group_item: rx.el.section(
                        rx.el.h2(
                            group_item[0],
                            id=group_item[0],
                            class_name="text-2xl font-bold text-gray-800 mb-6 pt-4 scroll-mt-20",
                        ),
                        rx.el.div(
                ...
                rx.el.div(
                    rx.el.div(
                        ...
                        class_name="flex flex-col items-center justify-center p-8 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50",
                    ),
                    class_name="flex items-center justify-center h-full",
                ),
            ),
            class_name="ml-64 pt-20 px-8 pb-8 h-[calc(100vh-5rem)]",
        ),
        results_modal(),
